...
main() {
  ...
  # Wait for the Ollama API to come up, then pull the requested model.
  ...
  if ollama_model_present "qwen2.5:32b-instruct"; then
    ...
    if ! ollama pull qwen2.5:32b-instruct; then
      echo "⚠ Failed to pull 'qwen2.5:32b-instruct'. Check internet/disk space/VRAM. You can retry later with:"
